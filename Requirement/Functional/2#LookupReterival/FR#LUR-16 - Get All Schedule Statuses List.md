## FR#LUR-16 – Get All Schedule Statuses List
...
- Return empty list if no statuses match filters
...
- Response must follow GetScheduleStatusListResponse schema

### API Level Business Rules
Result Name | ResultType | Response and ActionsValidation | Example | Should be logged | Response Category
------------|------------|--------------------------------|---------|------------------|------------------
SCHEDULE_STATUS_LIST_SUCCESS | Success | Returns paginated list | {"successCode": "SCHEDULE_STATUS_LIST_SUCCESS", "successMessage": "Schedule statuses retrieved successfully", "data": {...}} | No | Informational
UNAUTHORIZED_ERROR | Failure | Returns error code and message | {"ErrorCode": "UNAUTHORIZED_ERROR", "Message": "You are not authorized to view schedule statuses"} | Yes | Error
INVALID_PARAMETERS | Failure | Returns validation errors | {"ErrorCode": "INVALID_PARAMETERS", "Message": "Invalid page number"} | Yes | Error
SYSTEM_ERROR | Failure | Returns error code and message | {"ErrorCode": "SYSTEM_ERROR", "Message": "Failed to retrieve schedule statuses"} | Yes | Critical
...
1. **View All Schedule Statuses**
   ```
   As an authorized user
   I want to retrieve a list of schedule statuses
   So that I can view and select from available options
   ```

2. **Filter Active Statuses**
   ```
   As a regular user
   I want to see only active schedule statuses
...
3. **Search Statuses**
   ```
   As an authorized user
   I want to search schedule statuses by name
   So that I can quickly find specific statuses
   ```

4. **Navigate Pages**
   ```
   As an authorized user
   I want to navigate through pages of schedule statuses
   So that I can view all available options
   ```

### Glossary
- [Schedule Status] Current state of an interview schedule (e.g., Scheduled, Completed, Cancelled)
- [Pagination] Division of results into pages for efficient data retrieval
- [Search Term] Text to filter statuses by name
- [Active Status] Whether a schedule status is currently in use