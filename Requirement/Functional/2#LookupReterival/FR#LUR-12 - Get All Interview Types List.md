## FR#LUR-12 – Get All Interview Types List

### Description
System should allow authorized users to retrieve a paginated list of all interview types with filtering options for active/inactive status.
...
- [All Roles] Can view list of interview types
...
- [Postconditions]
    - Filtered list of interview types is returned
...
----------|-------------|------|----------
`Items` | Array of interview types | array | [{ ... }]
...
- Response must follow GetInterviewTypeListResponse schema

### API Level Business Rules
Result Name | ResultType | Response and ActionsValidation | Example | Should be logged | Response Category
------------|------------|--------------------------------|---------|------------------|------------------
INTERVIEW_TYPE_LIST_SUCCESS | Success | Returns paginated list | {"successCode": "INTERVIEW_TYPE_LIST_SUCCESS", "successMessage": "Interview types retrieved successfully", "data": {...}} | No | Informational
UNAUTHORIZED_ERROR | Failure | Returns error code and message | {"ErrorCode": "UNAUTHORIZED_ERROR", "Message": "You are not authorized to view interview types"} | Yes | Error
INVALID_PARAMETERS | Failure | Returns validation errors | {"ErrorCode": "INVALID_PARAMETERS", "Message": "Invalid page number"} | Yes | Error
SYSTEM_ERROR | Failure | Returns error code and message | {"ErrorCode": "SYSTEM_ERROR", "Message": "Failed to retrieve interview types"} | Yes | Critical
...
1. **View All Interview Types**
   ```
   As an authorized user
   I want to retrieve a list of interview types
...
2. **Filter Active Types**
   ```
   As a regular user
   I want to see only active interview types
...
3. **Search Types**
   ```
   As an authorized user
   I want to search interview types by name
...
4. **Navigate Pages**
   ```
   As an authorized user
   I want to navigate through pages of interview types
   So that I can view all available options
   ```

### Glossary
- [Interview Type] Format of interview (e.g., WeekDay-Scheduled, WeekDay-Walkin)
- [Pagination] Division of results into pages for efficient data retrieval
- [Search Term] Text to filter types by name
- [Active Status] Whether an interview type is currently in use